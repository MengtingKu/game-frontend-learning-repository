...
2026 Game Frontend Learning Repository — 個人 HTML5 遊戲前端學習 repo，以每日筆記（`notes/dayXX.md`）記錄學習歷程。採「一個遊戲 = 一個獨立 Vite 專案」的模式，目前有 `projects/collect-stars/`（以 **Phaser 3 + TypeScript + Vite** 開發的星星收集練習專案，進度為 Day01：已將 Phaser 官方星星收集教學拆成 `BootScene → PreloadScene → GameScene` 三個 Class Scene，`managers/`、`objects/`、`config/`、`types/` 仍是空殼，未來會依需求加入結算畫面、加速等客製化功能）與 `projects/big-two/`（大老二，目前僅有 Scene 骨架，玩法未開始）。老虎機會是另一個獨立專案，規劃於第二週後開始。根目錄的 `index.html` 是列出所有遊戲專案的靜態 dashboard。
...
```bash
cd projects/collect-stars   # 或 cd projects/big-two
npm install       # 安裝依賴
npm run dev       # 啟動開發伺服器（collect-stars: :5173，big-two: :5174）
...
- 素材路徑：`public/assets/` 下有 `raw/`（原始 PNG）、`atlas/`（TexturePacker 產出的圖集）、`demo/`（Phaser 官方教學素材，屬暫時性，未來會被客製化素材取代）。`preload()` 用相對路徑載入（如 `'assets/demo/sky.png'`），對應 Vite 的 `public/` 根目錄。
...
- 新增遊戲一律建立新的 `projects/<name>/` 資料夾（比照 `collect-stars`／`big-two` 的骨架），不要塞進既有專案的 `Phaser.Game` 實例；新增後記得在根目錄 dashboard（`index.html`）加卡片，並依 `docs/ARCHITECTURE.md` 的 port 對照表指定不衝突的 dev port。
