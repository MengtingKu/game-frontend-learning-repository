...
│   └── assets/
│       ├── demo/                 # Phaser 官方教學素材（sky/platform/star/bomb/dude），暫時性，會被 slot game 素材取代
...
└── src/
    ...
    │   ├── PreloadScene.ts       # 已實作：preload() 載入素材，目前載入的是 demo 素材而非 slot game 素材
    │   ├── GameScene.ts          # 已實作：目前是 Phaser 官方「星星收集 + 避炸彈」教學的 TS Class 版本，尚非 slot game 邏輯
    │   └── UIScene.ts            # 空檔案（預留：UI 疊加層 Scene，如按鈕、分數、下注面板）
    ...
    │   └── GameManager.ts        # 預留：集中管理遊戲狀態（分數、下注、遊戲階段）
    ├── objects/                  # 全部空檔案，預留 GameObject 子類別
    │   ├── Button.ts             # 預留：可互動按鈕元件
    │   ├── Reel.ts                # 預留：老虎機捲軸（reel）
    │   ├── Symbol.ts             # 預留：老虎機符號（symbol）
    │   └── WinLine.ts            # 預留：中獎線（payline）視覺呈現
    ├── ui/                        # 空資料夾，預留 UI 元件
    └── types/
        └── index.ts               # 空檔案，預留共用型別定義
```

> 「空檔案」代表 Day00 建立專案骨架時先佔位，尚未實作。新增內容前請先讀 [FEATURES.md](./FEATURES.md) 確認目前進度，避免搶跑或做出與當天學習目標衝突的抽象。
...
**尚未串接的部分**（下一步規劃見 `notes/day01.md` 「明日預告」與 `docs/FEATURES.md`）：老虎機核心玩法（spin、reel 停止、payline 判定、下注）完全未開始，`managers/` 與 `objects/` 尚為空殼。
